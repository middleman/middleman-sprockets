Feature: Make Gem assets available to Sprocket's lookup paths
  An attempt is made to lookup all css, js, image, & font paths in asset gems to be appended to Sprocket's lookup paths. The expectation is for these gems to loosely follow a few conventions:
